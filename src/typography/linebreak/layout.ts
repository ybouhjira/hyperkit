...
export function adjustmentRatios(
  ...
  for (let b = 0; b < breakpoints.length - 1; b++) {
    ...
    const bStart = breakpoints[b]!;
    const bEnd = breakpoints[b + 1]!;
    const start = b === 0 ? bStart : bStart + 1;
    for (let p = start; p <= bEnd; p++) {
      const item = items[p]!;
      if (item.type === 'box') {
        actualWidth += item.width;
      } else if (item.type === 'glue' && p !== start && p !== bEnd) {
        actualWidth += item.width;
        lineShrink += item.shrink;
        lineStretch += item.stretch;
      } else if (item.type === 'penalty' && p === bEnd) {
        actualWidth += item.width;
      }
    }
  ...
  for (let b = 0; b < breakpoints.length - 1; b++) {
    ...
    for (let p = start; p <= bEnd; p++) {
      ...
      if (item.type === 'box') {
        ...
        xOffset += gap;
      } else if (item.type === 'penalty' && p === bEnd && item.width > 0) {
        result.push({
          item: p,
          line: b,
          xOffset,
