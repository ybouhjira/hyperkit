import type { ShapeDefinition } from './shape-definition';
import { registerShape, getShapeOrDefault } from './shape-definition';
...
const cylinderShape: ShapeDefinition = {
  ...
  getBoundaryPoint: (node, ext) => {
    ...
    return getShapeOrDefault('rectangle').getBoundaryPoint(node, ext);
  },
  getPath: (node) => {
    const { x, y } = node.position;
    const { width: w, height: h } = node.size;
...
const cloudShape: ShapeDefinition = {
  ...
  getBoundaryPoint: (node, ext) => getShapeOrDefault('ellipse').getBoundaryPoint(node, ext),
  getPath: (node) => {
    const { x, y } = node.position;
    const { width: w, height: h } = node.size;
