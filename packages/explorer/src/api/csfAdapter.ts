...
export function convertCSFModule(
  ...
  const controls = convertArgTypes(meta.argTypes ?? {})
 ...
 */
function convertArgTypes(argTypes: Record<string, CSFArgType>): Record<string, ControlDef> {
  ...
  for (const [name, argType] of Object.entries(argTypes)) {
    ...
    const defaultValue = getDefaultValue(argType, controlType)
 ...
 * Checks multiple locations: argType.defaultValue.summary, argType.table.defaultValue.summary.
 * Falls back to reasonable defaults based on control type.
 */
function getDefaultValue(argType: CSFArgType, controlType: string): unknown {
  ...
  // Fallback defaults
  if (controlType === 'select' && argType.options && argType.options.length > 0) {
    return argType.options[0]
  }
